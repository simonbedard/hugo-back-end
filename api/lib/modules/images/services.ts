...
export class ImagesProvider {
    ...
    constructor({sample}){
        ...
        this.sample = sample; // Should we use sample data of make the real fetch request
    }
    public Extract({ providers, query, page }){
        const extract = (this.sample ? this.ExtractImagesFromSample({providers}) : this.ExtractImagesFromApis({providers, query, page}) );
        return extract;
    }

    // Extract images from test/sample data
    private ExtractImagesFromSample({ providers  }): Promise<any>{
    ...
    // Extract image from real api query
    private async ExtractImagesFromApis({ providers, query, page }): Promise<any>{

        return new Promise(function(resolve, reject) {
            const ExtractedImages: Array<ImageModel> = [];
            const ProviderPromises: Array<Promise<any>> = [];
